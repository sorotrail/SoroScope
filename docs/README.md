...
SoroScope is a browsable UI and read-only API for exploring Soroban contracts and their decoded events. Search for a contract, browse its activity in reverse-chronological order, and inspect any event's decoded topics and value — all through a lightweight, self-hostable web interface.

## Why SoroScope exists

Stellar's general-purpose explorers are built around transactions and payments; decoded **contract event** data — what a Soroban contract emitted, with human-readable topics and values — is hard to browse. SoroScope fills that gap: point it at a network (or at a [SoroTrail](https://github.com/sorotrail/SoroTrail) instance) and get a focused, searchable window into contract activity.
...
- **[Contributing](contributing.md)** — SoroScope is built to be extended

## Project links

- Source: [github.com/sorotrail/SoroScope](https://github.com/sorotrail)
- Issues & contribution opportunities: [GitHub Issues](https://github.com/sorotrail)
- Sister projects, same org: **SoroTrail** (indexes events) and **SoroBeacon** (alerts on them) — SoroScope is the third leg: explores them.

SoroScope is licensed under Apache-2.0.
